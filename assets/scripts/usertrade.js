document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("formEditar").addEventListener("submit", (e) => {
    ...
    filaSeleccionada.cells[1].textContent =
      document.getElementById("editUser1").value;
    filaSeleccionada.cells[2].textContent =
      document.getElementById("editUser2").value;
    filaSeleccionada.cells[3].textContent = document.getElementById(
      "editCursoSolicitado"
    ).value;
    filaSeleccionada.cells[4].textContent =
      document.getElementById("editCursoOfrecido").value;
    const estado = document.getElementById("editEstado").value;
    filaSeleccionada.cells[5].innerHTML = `<span class="badge bg-${
      estado === "Aceptado"
        ? "success"
        : estado === "Rechazado"
        ? "danger"
        : "warning"
    } text-dark">${estado}</span>`;
